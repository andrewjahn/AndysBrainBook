...
To format the timing files, download `this script <https://github.com/andrewjahn/FSL_Scripts/blob/master/make_FSL_Timings.sh>`__. (You can download it by clicking on the ``Raw`` button, then right-clicking in the newly-opened window and selecting "Save As"; for recent version of Github, just click the ``Download raw file`` button.) We won't go into detail about how it works, but all you need to do is place it in the experimental folder containing the subjects. Navigate to that folder using your terminal, and type ``chmod +x run_1stLevel_Analysis.sh`` to make it executable, if you are otherwise unable to run it. Then type ``bash make_FSL_Timings.sh``, or, alternatively, ``./run_1stLevel_Analysis.sh``. This will creating timing files for each run for each subject. To check the output, type ``cat sub-08/func/incongruent_run1.txt``. You should see numbers similar to the ones in the figure above.
