...
In this tutorial, we will download data from the Alzheimer's Disease Neuroimaging Initiative (ADNI), one of the databases that uses HCP protocols and is available through the Image and Data Archive (IDA). Along the way, you will learn how to use tools developed by the HCP, such as the Connectome Workbench. By the end of the walkthrough, you will be able to download other HCP datasets and analyze them using standardized preprocessing pipelines, and create new analyses using the large batteries of individual difference measures available on the HCP website.
