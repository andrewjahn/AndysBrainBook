...
When it has finished, go back to the SPM GUI and select ``Normalise -> Write``. Select the file ``y_rsub-1_T1.nii`` from the subject's ``anat`` directory, and for ``Images to Write``, select the ``res_accuracy_minus_chance.nii`` file in the directory ``SPM_Results_``1. Click ``Done``, and then click the Green Go button. The normalized classification map, which now has a ``w`` prepended to it, can be overlaid on an MNI template of your choosing to make sure that it was normalized properly.
